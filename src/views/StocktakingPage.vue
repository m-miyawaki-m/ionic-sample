  ...
  >
    ...
    <template v-if="layout === 'vertical'">
      <ion-list class="ion-margin-top">
        ...
        <NumberInput v-model="form.actualQuantity" label="実数量" placeholder="数量を入力" :min="0" />
      </ion-list>
    ...
    <template v-if="layout === 'stepper'">
      ...
      <ion-list class="ion-margin-top">
        ...
        <NumberInput v-if="steps[currentStep].field === 'actualQuantity'"
          v-model="form.actualQuantity" label="実数量" placeholder="数量を入力" :min="0" />
      </ion-list>
      ...
      <div class="ion-padding-horizontal ion-margin-top" style="display:flex;gap:8px;">
        ...
        <SubmitButton v-else label="登録" :loading="loading" @submit="submit" />
      </div>
    </template>
...
import type { StocktakingItem, MenuAction, ParsedScanCode } from '@/types';

type LayoutType = 'vertical' | 'grouped' | 'stepper';

const layout = ref<LayoutType>((localStorage.getItem('stocktakingLayout') as LayoutType) || 'vertical');
const currentStep = ref(0);

const steps = [
  { field: 'location', label: 'ロケーション' },
  { field: 'itemCode', label: '品目コード' },
  { field: 'actualQuantity', label: '実数量' },
];
...
const onMenuSelect = (action: string) => {
  if (['vertical', 'grouped', 'stepper'].includes(action)) {
    ...
    localStorage.setItem('stocktakingLayout', action);
    currentStep.value = 0;
...
const form = reactive<StocktakingItem>({ location: '', itemCode: '', actualQuantity: 0 });
...
const submit = async () => {
  ...
  if (res.success) {
    ...
    form.location = ''; form.itemCode = ''; form.actualQuantity = 0;
    currentStep.value = 0;
  } else {
